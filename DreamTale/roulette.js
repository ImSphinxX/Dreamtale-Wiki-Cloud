...
//https://twitter.com/zed_dash/status/1445191696715821058

let enabled = true;

const items = 9;
const wheels: Element[] = [1, 2, 3].map((x) =>
...
const all_equal = ([head, ...tail]: number[] = []) =>
  tail.every((x) => x == head);
const round_and_modulus_numbers =
  (numbers: number[], mod:number) =>
    numbers.map((x) => Math.round(x) % items);

const minimum_spins = 3;
const state_increment =
  (current: number, index: number) =>
    (minimum_spins + Math.random()) * items //minimum spins with 0-1 extra
    * ((index + 2) / 3)
...
const background_position_value = (position: number) =>
  Math.round(position) * row_height;
const background_position = (position: number) =>
...
const after_every_spin = (fn: function=()=>{}) => {
  fn();
  tag.classList.remove(`disabled`);
  enabled = true;
};
const run_after_spinning = (fn: function) =>
  setTimeout(() => after_every_spin(fn), 5000);
const run_after_shaking = (fn: function) =>
  setTimeout(fn, 500);

const spin = (state: number[]) => {
  before_every_spin();

  const next_state =
    state.map(state_increment);
